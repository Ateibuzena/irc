...
Server::Server(int port, ILineHandler *handler) : port_(port), listenFd_(-1), handler_(handler)
{
}
...
int Server::run() {
    ...
    if (listenFd_ < 0) { log_errno("socket"); return 1; }
    ...
    if (::bind(listenFd_, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
        log_errno("bind"); ::close(listenFd_); return 1;
    }
    if (::listen(listenFd_, 128) < 0) {
        log_errno("listen"); ::close(listenFd_); return 1;
    }

    try { setNonBlocking(listenFd_); }
    catch (const std::exception& e) { std::cerr << e.what() << "\n"; ::close(listenFd_); return 1; }
    ...
    while (true) {
        ...
        if (pfds_[0].revents & POLLIN) {
            for (;;) {
                ...
                if (cfd < 0) {
                    ...
                    log_errno("accept"); break;
                }
                try { setNonBlocking(cfd); }
        ...
        for (size_t i = 1; i < pfds_.size(); /* ++i o salto tras erase */) {
            int fd = pfds_[i].fd;
            ...
            if (re & POLLIN) {
                ...
                if (i >= pfds_.size()) break; // por seguridad si se vació
    ...
    for (;;) {
        ...
        if (n > 0) {
            ...
            while (findLineEnd(recvBuf_[fd], pos)) {
                ...
                // callback al parser (Persona 2)
                if (handler_) handler_->onLine(fd, line, *this);
            }
        } else if (n == 0) {
            ...
            if (errno == EAGAIN || errno == EWOULDBLOCK) break; // no hay más
            log_errno("recv");
            removeClientAtIndex(idx);
            return;
        }
...
void Server::handleWritable(size_t idx) {
    ...
    if (sendBuf_.find(fd) == sendBuf_.end()) {
...
void Server::queueSend(int fd, const std::string& line) {
